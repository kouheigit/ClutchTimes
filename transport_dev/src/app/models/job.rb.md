...
- `belongs_to :company` と `belongs_to :area` は別々に書く。
- `dependent: :destroy` のように、オプションは `:` を付けて書く。
- `enum` のハッシュは各要素をカンマで区切る。
- `status` enum は `published` という自動 scope を作るため、`scopes: false` を付けて手書きの `published` scope と衝突しないようにする。
- `search` scope は最後に必ず `jobs` を返す。
- `license_type_ids` は `joins(:license_types)` と `distinct` を使い、同じ求人が重複しないようにする。
- `keyword` は `sanitize_sql_like` を使うと、`%` や `_` が含まれる検索語でも意図しない LIKE 検索になりにくい。
